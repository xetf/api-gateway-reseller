...
log "Checking backup tooling"
if command_exists pg_dump; then
  printf 'pg_dump found: %s\n' "$(command -v pg_dump)"
elif command_exists docker && docker compose ps --status running postgres >/dev/null 2>&1; then
  printf 'Using docker compose postgres for backups.\n'
else
  die "No backup path found. Install pg_dump or run bundled Postgres with Docker Compose."
fi
